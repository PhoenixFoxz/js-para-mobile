...
### Métodos para interação em arrays: map, filter e reduce 

São métodos que permitem realizar operações diversas em elementos de array. Funcionam como `loops` especiais, executando uma função conhecida como `callback` para cada elemento dentro do array.
...
Percorre os elementos do array filtrando seu valores de acordo com uma ou mais condições, gerando um novo array de dados.
